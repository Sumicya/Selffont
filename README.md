...
> 状态：主机侧回归、Java 策略测试和诊断 APK 构建已通过 CI（提交 `16457fb`）；字体原版资源已校验。新 APK 的现代入口与 Firefox 的 Gecko 启动入口已有真机命中证据；目标字体的实际挂载和 Gecko 首选项注入现已由设备报告确认；用户已确认 Firefox 原问题和默认字体覆盖成功；补字覆盖与角标布局仍有待解决的问题。源码存在某个 Hook 入口、日志显示成功，不等于网页已使用目标字体。
