...
**修掉"派生字比旁边的字轻一笔"：每个自绘字都量厚度并回到本字重。**

- 实测：把部件缩进盒子会连笔画一起缩，赵 只有本面笔画的 0.51、见 0.60、维 0.59、进 0.66、迁 0.67、员 0.67、门 0.79、飞 0.86。这个字体没有"更轻的笔画"，所以规则跑完、存盘之前逐字量（墨面积÷轮廓周长），偏离本面基准 >12% 的沿法线偏移回基准（量一次修正一次），偏移量与前后比值写进报告；五个字重跑完规则字最差 0.95。
- 推不动的字：有参考字体时换用参考的轮廓并在报告里记 `replacedRecipe` 与原因；没有参考时报出来，不发轻一笔的字。
- 顺带修掉一个会**悄悄发空字形**的 bug：`TTGlyphPen.glyph()` 交出轮廓后会清空画笔，归一化时"量一个、再存第二个"会存进空轮廓（屏幕上就是豆腐块）。现在量到的对象就是存下的对象，并且存前拒绝空字形。新增 4 项契约测试（归一化达标、幂等、不存空字形、够不到的如实报告）。
- 参考字的容差外偏差不再导致丢字，改为记 `weightShortfall`（Black 的 8 个超密字 0.83–0.88 保留在字体里）。

## v2.1.0（2026-09-25）

**简体扩展闭环：自家规则拼得出的自己拼，拼不出的从可审计的参考字体借，五个面都覆盖 GB2312 6763/6763。**

- **参考源可审计**：新增 `config/reference-sources.json`（钉住项目、发布版本、文件、字节数、SHA-256、许可、保留字体名）与 `licenses/ChillRoundM-OFL.txt`；`tools/reference_font.py` 打开参考文件前核对字节数与 SHA-256，不符即拒（`UnpinnedReference`），未登记的一律不认。当前参考 = **寒蝉半圆体 ChillRoundM v1.805**（OFL-1.1，RFN `ChillRoundF`/`ChillRoundM`，本身基于 Zen Maru Gothic 调整，骨架与圆角风格同源）。
- **缺口实测**：Zen Maru 只画 GB2312 汉字 3378/6763（缺 3385）；v1.805 的 ChillRoundM 对这 3385 个字全部有真轮廓（早期版本是空占位，所以版本与哈希必须钉死）。
- **借入按字重缩放**：参考只有一个字重（400）。先量本字重与参考在共有探针字上的"墨面积÷轮廓周长"厚度比（Light 0.44 → Black 1.72），再沿轮廓法线做 miter 偏移（正数 = 加墨：实体外扩、字腔收缩；方向由最大轮廓的绕向决定，字腔跟着一起缩，否则重字重会越加越细），初始值取厚度差的一半，然后**量一次修正一次**迭代到目标厚度（±0.5 单位）。会把轮廓翻面、压塌（面积趋零）或甩出画布的偏移自动退让（尖角 miter 上限 4 倍偏移量），退让量逐字记进报告；单字轮廓越出 ±1400 单位盒子 → 拒绝该字并记录。
- **结果**：五个字重各派生 3385 个字形（规则 17 + 参考 3368/3369），GB2312 覆盖率 6763/6763；借入厚度"达成÷目标"中位数 0.98–1.04、**100% 落在 ±25% 内**；坐标审计 0 字越出 1200 单位。14 个练习缺字全部由自家规则画出，只有 Bold/Black 的 `飞` 由参考补。
- **规则优先，绝不写两遍**：cmap 在字形通过校验时就地占位，规则跑完参考只补缺口；保存后断言"写进字体的新字形名集合 == 本次准备写的名字"（这条断言抓出过真实 bug——参考流程曾把规则派生过的字又借了一遍，留下 `uni8D1D.1` 这种不可达重复字形）。
- **契约测试**：新增 `tests/test_reference_contract.py`（15 项：pin 的 SHA/字节/未登记/缺文件四种拒绝、许可随仓库、借入不覆盖自家派生、只借缺字并如实记录参考里没有的字、报告含来源与厚度、退让与塌缩、尖角不甩飞、越界拒绝、advance 取自参考、GB2312 集合由 codec 生成）。合计 140 项 Python 测试全绿。
- **文档**：`docs/simplified-extension.md` 增加"参考字体：借可以，但必须可审计、且按字重缩放"与覆盖情况表，新增对照图 `docs/images/reference-borrowing.png`（左原生 / 中规则 / 右借入，五个字重）。
