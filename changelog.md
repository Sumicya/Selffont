...
- Gecko 回退修复（1.4-gecko-fallback）：`font.name-list` 由"覆盖成只有文渊"改为"前置保留原回退链"，不再对无 list 项造窄列表，不触碰 emoji 首选项；修复火狐中文渊缺字（如 Unicode 15/16 新 emoji）显示豆腐块，缺字回退回系统字体与彩色 emoji。
