...
object GeckoFontPolicy {
    ...
    fun apply(original: Map<String, Any>, fontVisible: Boolean): Map<String, Any> {
        ...
        for (language in LANGUAGES) {
            for (family in FAMILIES) {
                ...
                // Prefer the primary family as the default font for this generic/language.
                prefs[nameKey] = FAMILY
                // Prepend the primary family to the EXISTING fallback list so Gecko keeps every
                ...
                // primary-family-only list that would tofu anything it lacks.
    ...
    /** Put the primary family first while preserving the rest of the list. Null means leave as-is. */
    @JvmStatic
    fun prependFamily(list: String): String? {
        val trimmed = list.trim()
        if (trimmed.isEmpty()) return null
        // Already led by the primary family (idempotent re-application): keep unchanged.
        ...
        // Drop any later duplicate of the primary family: exactly once, at the front.
